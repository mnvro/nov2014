...
public class MonServeurTCP extends ServerSocket implements Runnable {
		...
		public void run() {
			while (!socket.isClosed()){
				try {
					...
					// Le client semble déconnecté
					// TODO........................................                                pas fini cette partie *********************
					//e.printStackTrace();
	...
	/* 
	 * 			*******************************	Les variables d'instance de la classe MonServeurTCP *******************************
	 * */
	...
	public MonServeurTCP(int numPort,ApplicationServeur applicationServeur) throws IOException {
		super(numPort);
		numPort = this.getLocalPort();
		this.applicationServeur = applicationServeur;
	...
	public void run() {
		...
		try {
			while( !this.isClosed()){  
				...
				applicationServeur.getjFrameServeur().getjTextAreaObjetsRecus().append("\nClient numéro "+(listeClients.size()-1)+" vient de se connecter");
